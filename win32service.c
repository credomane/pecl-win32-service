...
static PHP_FUNCTION(win32_start_service_ctrl_dispatcher)
{
    ...
    WIN32SERVICE_ZG(service_name) = estrdup(name);

    WIN32SERVICE_ZG(te)[0].lpServiceName = WIN32SERVICE_ZG(service_name);
    WIN32SERVICE_ZG(te)[0].lpServiceProc = service_main;
    WIN32SERVICE_ZG(event) = CreateEvent(NULL, TRUE, FALSE, NULL);

    WIN32SERVICE_ZG(svc_thread) = CreateThread(NULL, 0, svc_thread_proc, &WIN32SERVICE_ZG(svc_thread), 0, &WIN32SERVICE_ZG(svc_thread_id));

    if (WIN32SERVICE_ZG(svc_thread) == NULL) {
        php_error_docref(NULL, E_WARNING, "failed to start dispatcher thread");
        RETURN_FALSE;
    }

    if (WAIT_OBJECT_0 == WaitForSingleObject(WIN32SERVICE_ZG(event), 30000)) {
        if (WIN32SERVICE_ZG(code) == NO_ERROR) {
            RETURN_TRUE;
        } else {
            RETURN_LONG(WIN32SERVICE_ZG(code));
        }
    }
...
static PHP_FUNCTION(win32_set_service_status)
{
    ...
    if ((status == SERVICE_CONTINUE_PENDING) || (status == SERVICE_PAUSE_PENDING) || (status == SERVICE_START_PENDING) || (status == SERVICE_STOP_PENDING)) {
        WIN32SERVICE_ZG(st.dwCheckPoint) = checkpoint;
    }

    if (!SetServiceStatus(WIN32SERVICE_ZG(sh), &WIN32SERVICE_ZG(st))) {
...
#define STR_DETAIL(name, var, def) \
    if (NULL != (tmp = zend_hash_str_find(Z_ARRVAL_P(details), name, sizeof(name)-1))) { \
...
static PHP_FUNCTION(win32_delete_service)
{
    ...
    size_t machine_len, service_len;
    SC_HANDLE hsvc, hmgr;
...
{
    RETURN_LONG(WIN32SERVICE_ZG(args.dwControl));
}
/* }}} */
...
static zend_function_entry win32service_functions[] = {
...
{
    if (WIN32SERVICE_ZG(sh)) {
        WIN32SERVICE_ZG(st).dwCurrentState = SERVICE_STOPPED;
        SetServiceStatus(WIN32SERVICE_ZG(sh), &WIN32SERVICE_ZG(st));
        /* PostThreadMessage(WIN32SERVICE_ZG(svc_thread_id), WM_QUIT, 0, 0); */
    }
    if (WIN32SERVICE_ZG(svc_thread)) {
        WaitForSingleObject(WIN32SERVICE_ZG(svc_thread), 10000);
        CloseHandle(WIN32SERVICE_ZG(svc_thread));
    }
    if (WIN32SERVICE_ZG(event)) {
        CloseHandle(WIN32SERVICE_ZG(event));
    }
    if (WIN32SERVICE_ZG(service_name)) {
        efree(WIN32SERVICE_ZG(service_name));
    }
    return SUCCESS;
}
...
zend_module_entry win32service_module_entry = {
    ...
    PHP_WIN32SERVICE_EXTNAME,
    win32service_functions,
